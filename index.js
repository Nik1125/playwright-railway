...
app.post("/run", (req, res) => enqueue(async () => {
  ...
  try {
    if (action === "loginCheck") {
    ...
    else if (action === "exportCookiesForSheets") {
      await page.goto("https://www.instagram.com/", { waitUntil: "domcontentloaded" }).catch(()=>{});
      const apiStatus = await page.evaluate(async () => {
        try { const r = await fetch("/api/v1/accounts/edit/web_form_data/", { credentials: "include" }); return r.status; }
        catch { return -1; }
      });
      out.apiStatus = apiStatus;
      out.isLoggedIn = apiStatus === 200;
      const all = await context.cookies("https://www.instagram.com").catch(()=>[]);
      const pick = (n) => (all.find(c => c.name === n)?.value || "");
      out.sheetRow = {
        updated_at: new Date().toISOString(),
        user_agent: DEFAULT_UA,
        is_logged_in: out.isLoggedIn,
        sessionid:  pick("sessionid"),
        csrftoken:  pick("csrftoken"),
        ds_user_id: pick("ds_user_id"),
        mid:        pick("mid"),
        ig_did:     pick("ig_did"),
      };
      out.ok = true;
    }
    else if (action === "loginWithCredentials") {
      const login = String(req.body?.login || req.body?.username || "").trim();
      const password = String(req.body?.password || "");
      const otp = (req.body?.otp ?? req.body?.twoFactorCode ?? "") + "";
      if (!login || !password) throw new Error("login and password required");

      await page.goto("https://www.instagram.com/accounts/login/", { waitUntil: "domcontentloaded" });
      // заполнить логин/пароль
      const userInput = page.locator('input[name="username"], input[aria-label*="Телефон" i], input[aria-label*="Username" i]').first();
      const passInput = page.locator('input[name="password"], input[aria-label*="Пароль" i], input[aria-label*="Password" i]').first();
      await userInput.fill(login).catch(()=>{});
      await passInput.fill(password).catch(()=>{});
      const loginBtn = page.locator('button:has-text("Войти"), button:has-text("Log in"), [role="button"]:has-text("Войти")').first();
      await Promise.race([
        page.waitForNavigation({ waitUntil: "domcontentloaded" }).catch(()=>{}),
      ...
      // 2FA?
      const codeInput = page.locator('input[name*="verification" i], input[name*="security" i], input[type="number"]').first();
      const needs2fa = await codeInput.count().catch(()=>0);
      if (needs2fa) {
        if (!otp) {
          out.need2FA = true; out.ok = false; out.step = "otp_required"; out.url = page.url();
          return res.json(out);
        }
        await codeInput.fill(otp).catch(()=>{});
        const submitOtp = page.locator('button:has-text("Далее"), button:has-text("Confirm"), button:has-text("Submit")').first();
        await Promise.race([
          page.waitForNavigation({ waitUntil: "domcontentloaded" }).catch(()=>{}),
          submitOtp.click().catch(()=>{})
        ]);
      }

      // закрыть «Сохранить данные/Включить уведомления» если появится
      const notNow = page.locator('button:has-text("Не сейчас"), div[role="button"]:has-text("Не сейчас"), button:has-text("Not now")').first();
      await notNow.click({ timeout: 2000 }).catch(()=>{});

      // проверить авторизацию через приватное API
      const apiStatus = await page.evaluate(async () => {
        try { const r = await fetch("/api/v1/accounts/edit/web_form_data/", { credentials: "include" }); return r.status; }
        catch { return -1; }
      });
